    ...
    <div class="row mt-5">
        <div class="col-12">
                ...
                <div class="d-flex my-3 align-items-center justify-content-between">
                    ...
                    <div class="d-flex align-items-center">
                        <select class="form-select ms-3" id="items-per-page">
                            <option value="5" {{ request()->get('per_page') == 5 ? 'selected' : '' }}>5 per page</option>
                            <option value="10" {{ request()->get('per_page') == 10 ? 'selected' : '' }}>10 per page</option>
                                                    ...
                                                    <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
                                                        <div class="carousel-inner">
                                                            <div class="carousel-item active">
                                                                <img src="{{ asset($template->product->photo) }}" class="d-block w-100" alt="Front">
                                                            </div>
                                                            <div class="carousel-item">
                                                                <img src="{{ asset($template->product->photo) }}" class="d-block w-100" alt="Back">
                                                            </div>
                                                        </div>
                                            ...
                                            <div class="col-md-7 d-flex flex-column">
                                                <div class="ms-3">
                                                    <div class="mb-4 mt-2">
                                                        ...
                                                        <p class="text-muted">Product Name: {{ $template->product->title }}</p>
                                                        <p class="text-muted">Size: {{ $template->product->size }}</p>
                                                        <p class="text-muted">Created on: {{ $template->product->created_at }}</p>
                                                        <p class="text-muted">Last edited on: {{ $template->product->updated_at }}</p>
                                                    </div>
                                                    <div class="d-flex align-items-center">
                                                        <button type="button" class="btn btn-primary">Add to basket</button>
                                                        <div class="ms-4">
                                                            <span class="text-muted">1 starting at </span>
                                                            <span class="text-decoration-line-through">Rs {{ $template->product->price - 10 }}</span>
                                                            <span class="text-success">Rs {{ $template->product->price }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-2 d-flex flex-column justify-content-around p-4">
                                                <div class="d-flex align-item-center">
                                                    <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        ...
                                                        <path d="M14.3267 18.0789C14.3267 18.7952 13.7436 19.3415 13.0481 19.3415H2.93378C2.21919 19.3415 1.65516 18.7761 1.65516 18.0789V7.902C1.65516 7.20485 2.23829 6.63946 2.93378 6.63946H7.10404L8.73958 5H2.93382C1.31663 5 0.000488281 6.30008 0.000488281 7.90193V18.0788C0.000488281 19.6999 1.31656 21 2.93382 21H13.0672C14.6843 21 16.0005 19.6999 16.0005 18.0788V12.2375L14.365 13.877L14.3642 18.0789Z" fill="black"></path>
                                                    </svg>
                                                    <a href="/editor-vue/{{ $template->product->id }}" class="btn btn-link">Edit</a>
                                                </div>
                                                <div class="d-flex align-item-center">
                                                    ...
                                                    <button class="btn btn-link copy-button" data-product-id="{{ $template->product->id }}">Copy</button>
                                                </div>
                                                <div class="d-flex align-item-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="20" height="20">
                                                        <path d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0L312.8 17.7C308.3 6.7 297 0 285.3 0l-122.7 0C151 0 139.7 6.7 135.2 17.7zM400 128L48 128c-8.8 0-16 7.2-16 16l0 352c0 8.8 7.2 16 16 16L400 512c8.8 0 16-7.2 16-16l0-352C416 135.2 408.8 128 400 128zM232 432c-13.3 0-24-10.7-24-24L208 264c0-13.3 10.7-24 24-24s24 10.7 24 24l0 144C256 421.3 245.3 432 232 432zM320 432c-13.3 0-24-10.7-24-24L296 264c0-13.3 10.7-24 24-24s24 10.7 24 24l0 144C344 421.3 333.3 432 320 432zM144 432c-13.3 0-24-10.7-24-24L120 264c0-13.3 10.7-24 24-24s24 10.7 24 24l0 144C168 421.3 157.3 432 144 432z"/>
                                                    </svg>
                                                    <button class="btn btn-link remove-button" data-product-id="{{ $template->product->id }}">Remove</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="container mt-3 d-flex justify-content-between">
                <div>
                    Showing {{ $userTemplates->firstItem() }} to {{ $userTemplates->lastItem() }} of {{ $userTemplates->total() }} entries
                </div>
                <div class="pagination-wrapper d-flex align-items-center justify-content-center">
                    {{ $userTemplates->appends(['per_page' => request()->get('per_page')])->links() }}
                </div>
            </div>
        </div>
    </div>
